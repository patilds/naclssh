...
def MySend(request_path, payload=None,
     ...
     try:
         ...
         if type(e) == urllib2.HTTPError and e.code == 403:	# forbidden, it happens
         	raise
         print >>sys.stderr, "Loading "+str(request_path)+": "+ExceptionDetail()+"; trying again in 2 seconds."
     time.sleep(2)
     return MySend1(request_path, payload, content_type, timeout, force_auth, **kwargs)
...
class MercurialVCS(VersionControlSystem):
  ...
  def __init__(self, options, repo_dir):
    ...
    print "cwd: %s" % cwd
    print "self.repo_dir: [%s]" % self.repo_dir
    print "self.repo_dir: {%s}" % self.repo_dir
  ...
  def GenerateDiff(self, extra_args):
    ...
    print "self.base_rev: %s" % self.base_rev
